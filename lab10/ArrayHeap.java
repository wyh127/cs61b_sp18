...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        /** TODO: Your code here. */
        int pind = parentIndex(index);
        // all indices greater than 0
        while(min(index, pind) != pind && pind > 0) {
            swap(index, pind);
            index = parentIndex(index);
            pind = parentIndex(index);
        }
    ...
    private void sink(int index) {
        ...
        /** TODO: Your code here. */
        int r_ind = rightIndex(index);
        int l_ind = leftIndex(index);

        while(min(index, r_ind) != index || min(index, l_ind) != index) {
            if(min(r_ind, l_ind) != r_ind) {
                swap(index, l_ind);
                index = l_ind;
            }
            else {
            ...
            if(leftIndex(index) < contents.length) {
                l_ind = leftIndex(index);
                r_ind = rightIndex(index);
            }
            else {
                break;
            }
